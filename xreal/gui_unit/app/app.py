...
from ..core.networking import IpcServer, EvalPlanStreamer
...
CALIB_W = CFG.eval.calibration_width
CALIB_H = CFG.eval.calibration_height
WIDTH_M = CFG.eval.display_width_m
HEIGHT_M = CFG.eval.display_height_m
DIST_M = CFG.eval.view_distance_m
...
CALIB_TARGET_COORDINATES = [ deg_to_px(xd, yd) for yd in ys_deg for xd in xs_deg ]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ipc_thread: Optional[IpcServer] = None
        ...
        self.ipc_status = QLabel("IPC: idle")
        self.step_label = QLabel("Step: idle")
        self.btn_connect_neon = QPushButton("Connect Neon")
        self.btn_start_ipc = QPushButton("Start IPC Server")
        ...
        ctrl_layout.addWidget(self.btn_start_ipc); ctrl_layout.addWidget(self.ipc_status)
        ...
        self.btn_start_ipc.clicked.connect(self.on_start_ipc)
    ...
    # ---- Helpers ----
    def status_ipc(self, msg):
        self.ipc_status.setText(f"IPC: {msg}")
    ...
    # ---- IPC ----
    def on_start_ipc(self):
        if self.ipc_thread and self.ipc_thread.conn:
            self.status_ipc("already connected")
            return
        self.ipc_thread = IpcServer(self.status_ipc)
        self.ipc_thread.start()
        subprocess.Popen(["openeye-xreal-processing"])
    ...
    def _deg_to_m(self, deg_x: float, deg_y: float, distance_m: float):
    ...
    def build_and_save_random_saccade_plan(self):
        ...
        while ti < frames_total:
            ...
            x_m, y_m = self._deg_to_m(deg_x, deg_y, distance_m=3.96)
            px = int(CALIB_W  / 2 + (x_m / (WIDTH_M  / 2)) * (CALIB_W / 2))
            py = int(CALIB_H  / 2 + (y_m / (HEIGHT_M  / 2)) * (CALIB_H / 2))
            for _ in range(frames_per_target):
                if ti >= frames_total:
                    break
                timeline.append({
                    "t_ms": int(t_ms),
                    "pos": {"x": float(px), "y": float(py)}
                })
                ti += 1
    ...
    # ---------- Mapping ----------
    def build_mapping(self, gaze_csv_path: str, window: int=CFG.mapping.window_width):
        if not os.path.exists(gaze_csv_path):
            return None
        df = pd.read_csv(gaze_csv_path)
        ev = df["event_log"].astype(str).fillna("")
        event_idxs = list(np.flatnonzero(ev == "event_log"))[1:]
        if not event_idxs:
            return None
